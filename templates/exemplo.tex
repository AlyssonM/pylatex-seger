...
Como pode ser visto na Figura \protect\ref{fig:grafico2}, {% if ultrapassagem_ocorre %} 
...
{% set rowspan = tabela_tarifas | length %}

{% for linha in tabela_tarifas %}
{% if loop.index0 == 0 %}
...
{% if linha.consumo_ponta == linha.consumo_fora_ponta %}
...
{% if linha.demanda_ponta == linha.demanda_fora %}
...
\fonte{Elaborado pelo autor}
\end{table} 
Após a consideração das premissas, houve  {% if atualizado_aumento %} 
...
que é necessário {%if demanda_aumento %}
...
\begin{table}[!ht]
    ...
    % \hline
    \begin{tabular}{|c|c|c{% if mostrar_bt %}|c{% endif %}|}
    \hline
    \textbf{DATA} & 
    \cellcolor{green!30}\textbf{A4 Verde Otimizada (R\$)} & 
    \cellcolor{blue!30}\textbf{A4 Azul Otimizada (R\$)} 
    {% if mostrar_bt %}& \cellcolor{red!20}\textbf{BT Optante -- B3 (R\$)} {% endif %} \\
    \hline
        
    % {% for linha in tabela_12meses_otimizados %}
    % {% if "TOTAL" in linha.data %}
    % \rowcolor{lightgray}
    ...
    {% if mostrar_bt %}& \textbf{R\$ {{ linha.bt }} }{% endif %} \\
    {% else %}
    \textbf{ {{ linha.data }} } & R\$ {{ linha.verde }} & R\$ {{ linha.azul }}
    {% if mostrar_bt %}& R\$ {{ linha.bt }}{% endif %} \\
    {% endif %}
    \hline
    {% endfor %}
    \end{tabular}
...
\begin{table}[!ht]
    ...
    {% for linha in tabela_contrato_comparado %}
